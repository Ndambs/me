...
js/resources-data.js    → NEW — the 66-book / 1,189-chapter resources catalogue
js/bible-study.js       → NEW — the reader app (search, progress, notes, nav)
images/bible-bg.jpg     → NEW — the background photo, used in both themes
```
...
- **The Resources Library**, now its own tab (labelled "Library" next to "Study"
  at the top) rather than something you scroll down to find — a full catalogue
  of all 66 books of the Bible (39 Old Testament + 27 New Testament), each one
  expandable to show every single chapter (1,189 in total). Every book has
  a "Read" button (opens that book's first chapter on BibleGateway in the
  NLT) and an "Overview" button (a plain-English summary on GotQuestions.org).
  Every chapter number is its own link to BibleHub.com, which puts dozens of
  Bible translations, cross-references, and classic/modern commentaries
  side by side for that exact chapter. There's a search box to jump straight
  to a book by name. All of these links point to real, existing pages I
  verified while building this — nothing here is a guessed or made-up URL.
- **A photo background, in both themes** — the open-Bible photo you provided
  now sits behind the whole app (`images/bible-bg.jpg`), tinted differently
  per theme so it reads as atmosphere rather than clutter: a dark, moody wash
  in dark mode, a warm parchment wash in light mode. The sidebar, top bar,
  and reading pane are all slightly translucent with a blur, so the photo
  shows through softly without ever competing with the text.
- **Auto-hiding top bar and sidebar** — scroll down inside either the Study
  or Library view and the top bar (with the theme toggle) slides up and out
  of view, and on desktop the sidebar collapses to give the reading pane the
  full width. Scroll up even slightly, or scroll near the top, and both
  reappear immediately. All of this respects the "reduce motion"
  accessibility setting — if someone has that turned on at the OS level, the
  transitions become instant instead of animated.
- **A "Study" / "Library" tab switcher** at the top of the page, so getting
  to the resources catalogue is one click instead of scrolling past the
  entire 274-day reader.
